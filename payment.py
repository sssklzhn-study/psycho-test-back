...
def generate_test_qr(amount: float, order_id: str) -> str:
    ...
    try:
        ...
        # Конвертируем в base64 - ИСПРАВЛЕННЫЙ КОД
        buffer = io.BytesIO()
        
        # Проверяем тип изображения и сохраняем соответствующим образом
        if hasattr(img, 'save'):
            # Если это PIL Image
            img.save(buffer, format='PNG')
        else:
            # Если это qrcode.image.pil.PilImage
        ...
        # В случае ошибки возвращаем текстовую заглушку
        return f"QR-DEMO-{order_id}"
